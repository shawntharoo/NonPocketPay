...
{
    class BlockExistingDeviceTest
    {

        [Test]

        public void TC2004973_BlockExistingDeviceTest()
        {
            globals.expRpt = new ExtentReporter();
            globals.expRpt.setupExtentReport("Automation Framework", "Non-Pocket framwork");
            IWebDriver driver = new ChromeDriver();
            globals.expRpt.CreateTest("Block Existing Device Test");
            ...
            using (JsonTextReader reader = new JsonTextReader(file))
            {
                ...
                globals.expRpt.logReportStatement(AventStack.ExtentReports.Status.Pass, "Click Block");
                devices.ClickBlock();

                globals.expRpt.logReportStatement(AventStack.ExtentReports.Status.Pass, "Click Confirm Block");
                devices.ClickCnfmBlock();

                globals.expRpt.logReportStatement(AventStack.ExtentReports.Status.Pass, "Click Yes");
                devices.ClickYes();

                Assert.IsTrue(driver.FindElement(By.XPath("//h2[contains(text(),'Devices')]")).Displayed);
                
            }
            globals.expRpt.logReportStatement(AventStack.ExtentReports.Status.Pass, "Exesting device blocked successfully");
