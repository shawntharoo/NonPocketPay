...
{
    class UploadSimManagement_MaintenanceTest
    {

       
        [Test]
        public void TC2005045_UploadsimManagement_MaintenanceTest()
        {
            ...
            using (JsonTextReader reader = new JsonTextReader(file))
            {
                ...
                //addsimManagement.setAdmSimNo(data["SimNumber"].ToString());

                Thread.Sleep(1000);
                globals.expRpt.logReportStatement(AventStack.ExtentReports.Status.Pass, "Click Add Sim Number");

                maintenance.uploadfile(data["UploadFile"].ToString());

                //Assert.AreEqual("Successfully deleted sim 9550555550", addsimManagement.getSucessfullyaddedText());
